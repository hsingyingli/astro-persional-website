    ...
    <div className="w-full h-full">
      <div className="p-4 flex items-center flex-col gap-2">
        <div className="relative w-[225px] h-[150px] sm:w-[450px] sm:h-[300px] rounded-lg overflow-hidden">
          <img src={project.image[0]} alt={project.title} className='w-full h-full object-fill' />
        </div>
          ...
          <ul>
            {
              fields.map((field) => {
                ...
                if (!Array.isArray(data)) {
                  ...
                  return <li key={field} className="my-3"><span className="p-1 rounded-sm bg-teal-300 bg-opacity-50">{field}</span>: <a href={url}>{url}</a></li>
                }
